...
while True:
        ...
        if img_count >= len(imgs_list):
            ...
            try:
                with serial.Serial(PORT_A, BAUD_RATE, timeout=2) as ser:
                    print(f"Main script connected to {ser.name}")
                    print("Main script sending object data...")
                    
                    # 1. Send each object as its own complete line
                    #    We loop from 0 to the end of the list
                    for i in range(len(labeles)):
                        
                        # Get the data for ONE object
                        labeln = labeles[i]
                        x_coord = round(centers_xR[i], 2)
                        y_coord = round(centers_yR[i], 2)
                        
                        # 2. Format it into a single comma-separated string as packet
                        message = f"{labeln},{x_coord},{y_coord}\n"
                        
                        # 3. Send the packet (and print what we're sending)
                        print(f"Sending: {message.strip()}") # .strip() for a clean print
                        ser.write(message.encode('utf-8'))
                        
                        # Optional: Wait for a tiny moment if needed
                        time.sleep(0.01) 

                    # 4. After the loop, send a "DONE" marker, this tells the ESP32 we are finished sending this batch.
                    print("Sending: DONE")
                    ser.write(b"DONE\n")

                    # 5. Now, wait for an acknowledgment from the ESP32
                    response = ser.readline()
                    
                    if response:
                        print(f"Main script received: {response.decode('utf-8').strip()}")
                    else:
                        print("Main script: No response from emulator.")
    ...
    # (The 'resize' variable is now always True)
    if resize == True:
        frame_resized = cv2.resize(frame, (resW, resH))
    else:
        frame_resized = frame # Should not happen, but good practice
    ...
    if key == ord('q') or key == ord('Q'):
        ...
        cv2.waitKey()
    elif key == ord('p') or key == ord('P'):  # Changed from 'p'
        # Saves exactly one frame each time 'o' is pressed
